...
Worker.prototype.readyForWork = function() {
    ...
    this.pool.getJob(function (data, doneCallback) {
        if (data === null) { // no more data, we can close this worker
            ...
            throw new Error('Worker #' + that.id + ' was already closed. You cannot reuse a closed worker!');
        } else {
            ...
            if (that.waitingTimeout) {
                ...
                try {
                    ...
                    if (result.status === 'success') {
                        ...
                        jobCallback(new Error(result.errMessage), result.data);
                    } else {
                        jobCallback(new Error('Communication error between Master and Worker (worker #' + that.id + ')'));
                        result.closing = true;
                        that.createProcess();
                    }
